...
if __name__ == '__main__':
    ...
    tfm = transforms.Compose([
        ...
        # Random vertical flip with a probability of 50%
        ...
        # Randomly crop the image to a size of 224x224
        # transforms.RandomResizedCrop(size=(224, 224), scale=(0.8, 1.0)),

        # Randomly apply a Gaussian blur with a probability of 50%
        # transforms.RandomApply([transforms.GaussianBlur(kernel_size=3)], p=0.5),

        # Resize to 224x224 if needed (some of the above transforms may change size)
        transforms.Resize((224, 224)),

        # Convert the image to a PyTorch tensor
        transforms.ToTensor(),

        # Normalize the tensor with mean and std values used for pre-trained models
    ...
    # 训练模型
    model = train_model(model, criterion, optimizer, train_loader, val_loader, num_epochs=50)
    # torch.save(model.state_dict(), 'model_real.pth')
    torch.save(model.state_dict(), 'model_real_augmentation.pth')
    # 在测试集上评估模型
    acc, f1, tpr = evaluate_model(model, test_loader)

    print(f"Test Accuracy: {acc:.4f}, F1 Score: {f1:.4f}")

    with open("no_synthetic_augmentation.txt", "a", encoding="utf-8") as file:
        file.write(
            "Test Accuracy:{}\nF1 Score:{}\n TPR:{}\n".format(acc, f1, tpr))
